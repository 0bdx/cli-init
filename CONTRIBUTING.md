...
3.  Check your global __TypeScript__ version:  
    ...
    - `tsc` is needed to generate the __cli-init.d.ts__ type declaration
...
`git clone git@github.com:0bdx/cli-init.git && cd cli-init`
...
Open the `cli-init` project in VS Code:  
`code .`
...
Build __cli-init.js__ and __cli-init.d.ts__:  
`npm run build:prod`  
`npm run build:types`

Run all tests on the built __cli-init.js__ file:  
`npm run preflight:test`

Check that __cli-init.js__ uses all types correctly:  
...
Publish to [npmjs.com/package/@0bdx/cli-init](
https://www.npmjs.com/package/@0bdx/cli-init):  
`npm publish`
...
```sh
touch cli-init.d.ts cli-init.js
```
...
9. Name the command, and point it to the start-script in the __bin/__ folder:  
   `sed -ix 's|§4|"bin": { "0bdx-init": "bin/0bdx-init.js" }|' *e.json`
10. The first script generates the main file, __cli-init.js__, and then  
    the second script generates the type declarations, __cli-init.d.ts__.  
    ...
    `sed -ix 's/§c/"§Z": "for s in {§A,§B};do npm run §Z:$s;done"/' *e.json`  
    `sed -ix 's/§A/prod/g;s/§B/types/g;s/§Z/build/g;' *e.json`  
11. The fourth script runs unit tests on the main file, __cli-init.js__,  
